...
4. **`README.md` is the index, and it has front matter.** Do not add an `index.md`, do not
   introduce a redirect, and do not strip the fence -- `permalink: /` is what makes the README
   build to `index.html`. See `EXHIBIT.md`.
5. **Do not resolve `OPEN.md` §1 locally.** Whether `library` subsumes `bottles` touches D4 and D11
...
6. **`adoption/engines.yml` is a catalogue, never an authority.** Every row carries `declared:`.
...
7. **`BOTTLES.md` is a consumer's requirements, not a spec.** If it starts describing how a bottle
